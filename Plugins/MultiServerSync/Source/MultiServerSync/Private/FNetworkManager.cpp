...
void FNetworkManager::ProcessReceivedData(const TArray<uint8>& Data, const FIPv4Endpoint& Sender)
{
    ...
    case ENetworkMessageType::SettingsResponse:
        ...
        break;
    case ENetworkMessageType::SettingsUpdate:
        HandleSettingsUpdateMessage(Message, Sender);
        break;
    case ENetworkMessageType::SettingsAck:
        HandleSettingsAckMessage(Message, Sender);
        break;
    default:
...
bool FNetworkManager::SendSettingsMessage(const TArray<uint8>& SettingsData, ENetworkMessageType SettingsMsgType)
{
    if (!bIsInitialized)
    {
        return false;
    ...
    // 유효한 설정 메시지 타입인지 확인
    if (SettingsMsgType != ENetworkMessageType::SettingsSync &&
        SettingsMsgType != ENetworkMessageType::SettingsRequest &&
        SettingsMsgType != ENetworkMessageType::SettingsResponse &&
        SettingsMsgType != ENetworkMessageType::SettingsUpdate &&
        SettingsMsgType != ENetworkMessageType::SettingsAck)
    {
        UE_LOG(LogMultiServerSync, Warning, TEXT("Invalid settings message type"));
        return false;
    }
...
bool FNetworkManager::BroadcastSettingsMessage(const TArray<uint8>& SettingsData, ENetworkMessageType SettingsMsgType)
{
    return SendSettingsMessage(SettingsData, SettingsMsgType);
}

bool FNetworkManager::SendSettingsToServer(const FString& ServerId, const TArray<uint8>& SettingsData, ENetworkMessageType SettingsMsgType)
{
    if (!bIsInitialized)
    {
        return false;
    ...
    // 서버 엔드포인트 찾기
    FServerEndpoint* ServerInfo = DiscoveredServers.Find(ServerId);
    if (!ServerInfo)
    {
        UE_LOG(LogMultiServerSync, Warning, TEXT("Server not found: %s"), *ServerId);
        return false;
    }
    ...
    // 해당 서버로 메시지 전송
    FIPv4Endpoint Endpoint(ServerInfo->IPAddress, ServerInfo->Port);
...
void FNetworkManager::HandleSettingsSyncMessage(const FNetworkMessage& Message, const FIPv4Endpoint& Sender)
{
    ...
    UE_LOG(LogMultiServerSync, Verbose, TEXT("Received settings sync from: %s"), *SenderId);

    // 설정 핸들러가 등록되어 있으면 호출
    if (SettingsMessageHandler)
    {
        SettingsMessageHandler(Message.GetData(), SenderId);
    }

    // 확인 응답 전송
    if (!bIsMaster) // 슬레이브만 확인 응답 전송
    {
        TArray<uint8> AckData;
        AckData.AddUninitialized(4);
        FMemory::Memcpy(AckData.GetData(), &Message.GetSequenceNumber(), 4);

        // ACK 메시지 생성 및 발신자에게 전송
        FNetworkMessage AckMessage(ENetworkMessageType::SettingsAck, AckData);
...
void FNetworkManager::HandleSettingsRequestMessage(const FNetworkMessage& Message, const FIPv4Endpoint& Sender)
{
    ...
    UE_LOG(LogMultiServerSync, Verbose, TEXT("Received settings request from: %s"), *SenderId);
...
void FNetworkManager::HandleSettingsResponseMessage(const FNetworkMessage& Message, const FIPv4Endpoint& Sender)
{
    ...
    UE_LOG(LogMultiServerSync, Verbose, TEXT("Received settings response from: %s"), *SenderId);
...
void FNetworkManager::HandleSettingsUpdateMessage(const FNetworkMessage& Message, const FIPv4Endpoint& Sender)
{
    // 발신자 ID 찾기
    FString SenderId;
    for (const auto& Pair : DiscoveredServers)
    ...
    UE_LOG(LogMultiServerSync, Verbose, TEXT("Received settings update from: %s"), *SenderId);
...
void FNetworkManager::HandleSettingsAckMessage(const FNetworkMessage& Message, const FIPv4Endpoint& Sender)
{
    // 발신자 ID 찾기
    FString SenderId;
    for (const auto& Pair : DiscoveredServers)
    ...
    if (Message.GetData().Num() >= 4)
    {
        FMemory::Memcpy(&AckedSequence, Message.GetData().GetData(), 4);
    }

    UE_LOG(LogMultiServerSync, Verbose, TEXT("Received settings ACK from %s for sequence %u"), *SenderId, AckedSequence);

    // 여기서 추가 작업 가능 (예: ACK 시퀀스 추적)
}